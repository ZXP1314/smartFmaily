...
- (void)commonInit
{
    ...
    self.backgroundColor = [UIColor clearColor];//画曲线的视图颜色
...
- (void)setDefaultParameters
{
    ...
    _lineWidth = 1;//曲线的宽
    _innerGridLineWidth = 0;//横纵线条的宽
    _axisLineWidth = 1;//横纵坐标的宽
    ...
    _indexLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:10];//横坐标字体大小
    
    _valueLabelBackgroundColor = [UIColor colorWithWhite:1 alpha:0.75];
    _valueLabelTextColor = [UIColor grayColor];
    _valueLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:10];//纵坐标字体大小
...
- (UILabel*)createLabelForIndex: (NSUInteger)index
{
    ...
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - 4.0f, p.y + 7, width + 2, 14)];
    label.text = text;
