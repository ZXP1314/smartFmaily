...
- (void)setHandleSize:(CGPoint)handleSize {
...
- (void)initialSetup {
    ...
    // handle
    self.handleSize = CGPointMake(20, 20);
    self.handlePosition = 10;
    self.handleImage = [UIImage imageNamed:@"hand"];
}
...
- (void)updateSetup
{
    if (_handleView) {
        CGRect handleViewFrame =
                CGRectMake(_origin.x - _handleSize.x / 2, _origin.y - _radius  - _handleSize.y / 2,
                        _handleSize.x, _radius * 2 + _handleSize.y);
        _handleView.frame = handleViewFrame;
    }
    if (_handleImageView) {
        _handleImageView.frame = CGRectMake(0, _handlePosition,_handleSize.x,_handleSize.y);
    }
    if (_handleImageView) {
...
- (void)drawArc:(CGFloat)value inContext:(CGContextRef)context
{
    ...
    */
    ...
    CGContextBeginPath(context);
    CGContextSetLineWidth(context, self.handleSize.y*0.8);
    CGContextSetRGBStrokeColor(context, 0.5, 0.5, 0.5, _trackAlpha);//改变画笔颜色
    ...
    CGContextAddArc(context, _origin.x, _origin.y, _radius-self.handleSize.y/2, startAngle, endAngle, NO);
